...
public final class BulkIngestMapFileLoader implements Runnable {
    ...
    private Path distCpDirectory(Path jobDirectory) throws Exception {
        ...
        if (!src.equals(dest)) {
            ...
            DistCpOptions options = new DistCpOptions.Builder(srcPath, destPath).withLogPath(logPath).withSyncFolder(true)
                            .preserve(DistCpOptions.FileAttribute.USER).preserve(DistCpOptions.FileAttribute.GROUP)
                            .preserve(DistCpOptions.FileAttribute.PERMISSION).build();
